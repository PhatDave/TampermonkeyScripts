...
function getLatestIlvlEntry(character) {
    return new Promise((resolve, reject) => {
        fetch(`https://indecisive-data.app.jet-black.xyz/api/collections/armory_character_snapshot/records?perPage=1&sort=-created&filter=(character.id='${character.id}')`, options).then(response => response.json()).then(response => {
                ...
                if (shouldUpdate) {
                    if (window.location.href.endsWith(record.freakzId)) {
                        waitForElement(document, "span.ilevel").then(elm => {
                            ...
                            fetch('' +
                                'https://indecisive-data.app.jet-black.xyz/api/collections/armory_character_item/records', postOptions)
                        });
                    } else {
